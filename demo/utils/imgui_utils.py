...
def show_properties_dialog(fb_width, fb_height, hidpi: bool):
    ...
    ImGui.text(
        f"Submit CPU {(stats.cpu_time_end - stats.cpu_time_begin) * to_ms_cpu:3.3f}, GPU {(stats.gpu_time_end - stats.gpu_time_begin) * to_ms_gpu:3.3f} (L: {stats.max_gpu_latency})"
    )
    ...
    if ImGui.collapsing_header("\uf12e Resources", 1 << 5):
        caps = bgfx.get_caps()
        item_height = ImGui.get_text_line_height_with_spacing() / res_multiplier
