...
class FluidSimulator:
    ...
    def add_velocity(self, position: tuple, velocity: tuple, radius: float):
        ...
        bgfx.setUniform(self.position_uniform,
                        as_void_ptr(np.array([position[0], position[1]]).astype(np.float32).tobytes()))
        bgfx.setUniform(self.value_uniform,
                        as_void_ptr(np.array([velocity[0], velocity[1]]).astype(np.float32).tobytes()))
    ...
    def add_circle_obstacle(self, position: tuple, radius: float, static=False):
        ...
        bgfx.setUniform(self.static_uniform,
                        as_void_ptr(np.array([1.0 if static else 0.0, 0.0, 0.0, 0.0]).astype(np.float32).tobytes()))
    ...
    def update(self, time_delta: float):
        if self.simulate:
            ...
            # Subtract gradient
            bgfx.dispatch(0, self._subtract_gradient_kernel, self._num_groups_x, self._num_groups_y, 1)
            self._flip_velocity_buffer()

            # Clear obstacles
            bgfx.setBuffer(TemplateConstants.GENERIC.value, self._obstacles_buffer, bgfx.Access.ReadWrite)
            bgfx.dispatch(0, self._clear_buffer_kernel, self._num_groups_x, self._num_groups_y, 1)
            bgfx.setBuffer(TemplateConstants.OBSTACLES.value, self._obstacles_buffer, bgfx.Access.ReadWrite)
    ...
    def _load_compute_kernels(self):
        self._add_velocity_kernel = bgfx.createProgram(load_shader("shader.AddVelocity.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._init_boundaries_kernel = bgfx.createProgram(load_shader("shader.InitBoundaries.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._advect_velocity_kernel = bgfx.createProgram(load_shader("shader.AdvectVelocity.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._divergence_kernel = bgfx.createProgram(load_shader("shader.Divergence.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._poisson_kernel = bgfx.createProgram(load_shader("shader.Poisson.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._subtract_gradient_kernel = bgfx.createProgram(load_shader("shader.SubtractGradient.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._calc_vorticity_kernel = bgfx.createProgram(load_shader("shader.CalcVorticity.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._apply_vorticity_kernel = bgfx.createProgram(load_shader("shader.ApplyVorticity.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._add_circle_obstacle_kernel = bgfx.createProgram(load_shader("shader.AddCircleObstacle.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._add_triangle_obstacle_kernel = bgfx.createProgram(load_shader("shader.AddTriangleObstacle.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._clear_buffer_kernel = bgfx.createProgram(load_shader("shader.ClearBuffer.comp", ShaderType.COMPUTE, root_path=root_path), True)
        self._viscosity_kernel = bgfx.createProgram(load_shader("shader.Viscosity.comp", ShaderType.COMPUTE, root_path=root_path), True)
